...
from ${all_tracks}
where case
    when trim('${inputs.name_of_input}') = '' then true
    else track ilike '%' || trim('${inputs.name_of_input}') || '%'
end
order by scrobbles desc
limit 10
```
...
where id = cast('${track_id_filter[0].track_id}' as integer)
```
...
where t.id = cast('${track_id_filter[0].track_id}' as integer)
...
from music.tracks t
join music.tracks p
  on p.id = cast('${track_id_filter[0].track_id}' as integer)
where
    trim(
        regexp_replace(
            regexp_replace(
...
join music.tracks t on t.id = s.track_id
where t.id = cast('${track_id_filter[0].track_id}' as integer)
  and t.album_name = (select album_name from music.tracks where id = cast('${track_id_filter[0].track_id}' as integer))
  and t.artist_id = (select artist_id from music.tracks where id = cast('${track_id_filter[0].track_id}' as integer))
group by t.name
order by plays desc
limit 10
```
